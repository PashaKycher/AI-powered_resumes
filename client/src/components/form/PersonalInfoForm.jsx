...
const PersonalInfoForm = ({ data, onChange, removeBackgraund, setRemoveBackgraund }) => {
        ...
        <div>
            ...
            <div className='flex items-center gap-2'>
                <label htmlFor='image'>
                    ...
                    <input type="file" name="image" onChange={(e) => handleChange("image", e.target.files[0])}
                        id="image" className="hidden" accept='image/jpeg, image/png' />
                </label>
                ...
                {typeof data.image === 'object' && (
                    <div className='flex- flex-col gap-1 pl-4 text-sm'>
                        ...
                        <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                            <input type="checkbox" className='sr-only peer' onChange={() => setRemoveBackgraund(prev => !prev)}
                                checked={removeBackgraund} />
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        className='mt-1 w-full px-3 py-2 border border-gray-300 founded-lg focus:ring
                        focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors text-sm'
                        required={field.required} placeholder={`Enter your ${field.label.toLowerCase()}`}/>
                    </div>
                )
            })} 
        </div>
